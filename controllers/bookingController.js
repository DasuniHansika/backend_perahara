...
exports.createBooking = async (req, res) => {
  try {
    ...
    console.log(
      `📋 Creating booking for user ${customerId} with cart items:`,
      cartItemIds
    );
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    const bookingResults = await query(
      `
      SELECT b.booking_id, b.customer_id, b.shop_id, b.status,
             s.seller_id, u.user_id as customer_user_id
      FROM bookings b
      JOIN shops s ON b.shop_id = s.shop_id
      JOIN users u ON b.customer_id = u.user_id
      WHERE b.booking_id = ?
...
/**
 * Delete booking (admin only or booking owner for cancellation)
 */
