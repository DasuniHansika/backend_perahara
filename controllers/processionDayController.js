// controllers/processionDayController.js
...
exports.getAvailableShops = async (req, res) => {
  try {
    ...
    // Build query to get available shops
    let sql = `
      SELECT s.shop_id, s.name, s.street, s.latitude, s.longitude,
             s.image1, s.image2, s.image3, s.image4, s.description,
             COALESCE(users.username, 'Unknown') as seller_username,
             COALESCE(CONCAT(sellers.first_name, ' ', sellers.last_name), 'Unknown') as seller_name,
             (
               SELECT MIN(sta.price)
               FROM seat_type_availability sta
               JOIN seat_types st ON sta.seat_type_id = st.seat_type_id
    ...
    if (minSeats && !isNaN(parseInt(minSeats))) {
      ...
      `;
      params.push(id, parseInt(minSeats));
    }
