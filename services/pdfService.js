...
class PDFService {
  ...
  /**
   * Generate ticket PDF with ticket number and QR code
   */
  async generateTicketPDF(ticketData) {
    ...
    try {
      const {
        ...
        payhereOrderId,
        payherePaymentId,
        ticketNumber, // Add ticket number to ticket data
      } = ticketData;

      // Validate that ticket number is provided
      if (!ticketNumber) {
        throw new Error("Ticket number is required for PDF generation");
      }
      ...
      // Generate unique filename using ticket number
  ...
  generateTicketHTML(ticketData) {
    ...
    <head>
        ...
        <title>Perahera Gallery Ticket</title>
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .seat-details { background: #0F172A; color: white; padding: 25px; border-radius: 4px; margin: 20px 0; text-align: center; }
            .seat-details h3 { margin-bottom: 15px; font-size: 20px; }
            .seat-info { display: flex; justify-content: space-around; flex-wrap: wrap; }
            .seat-info div { margin: 5px; }
            .seat-info .label { font-size: 12px; opacity: 0.8; }
            .seat-info .value { font-size: 18px; font-weight: bold; }
      ...
      customerInfo.lastName
                ...
                <div class="seat-details">
                    ...
                    <div class="seat-info">
                        <div>
                            ...
                            <div class="value">${
                              seatDetails.name || "Standard"
                            }</div>
                        </div>
                        <div>
                            <div class="label">QUANTITY</div>
                            <div class="value">${
                              seatDetails.quantity || 1
                            }</div>
                        </div>
                        <div>
                            <div class="label">EVENT DATE</div>
                            <div class="value">${
                              formatEventDate(seatDetails.eventDate) || "TBD"
                            }</div>
                        </div>
                        <div>
                            <div class="label">LOCATION</div>
                            <div class="value">${
                              seatDetails.location || "Kandy"
                            }</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="payment-summary">
                    ...
                    <div class="payment-row payment-total">
                        ...
                        <span>LKR ${paymentDetails.amount || "0.00"}</span>
                    </div>
                </div>
                `
                    : ""
